  ...
  if (!functions || functions.length === 0) {
    return (
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <h2 className="text-sm font-medium text-white mb-3">Top cursed functions</h2>
          ...
          <div key={fn.name} className="flex items-center gap-3">
            ...
            <div className="w-20 h-1.5 bg-gray-800 rounded-full shrink-0">
              <div
                ...
                style={{ width: `${fn.wtf_score}%` }}
              />
            </div>

            <span className={`text-xs font-medium w-7 text-right shrink-0 ${colors.text}`}>
              {fn.wtf_score}
            </span>
          </div>
        )
